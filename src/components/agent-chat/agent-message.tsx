...
export function AgentMessageView({ message }: { message: AgentMessage }) {
  ...
  const totalDuration =
    message.finishedAt && message.startedAt
      ? (message.finishedAt - message.startedAt) / 1000
      : undefined
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0">
        <div
          className={cn(
            ...
            running && "ring-2 ring-emerald-500/40 ring-offset-2 ring-offset-background"
          )}
        >
      ...
      <div className="min-w-0 flex-1">
          ...
          <div className="ml-auto flex items-center gap-3 text-[10px] text-muted-foreground">
            {message.iterations != null && message.iterations > 0 && (
              <span className="flex items-center gap-0.5 font-mono">
                ...
                {message.iterations} {message.iterations === 1 ? "iter" : "iters"}
              </span>
            )}
        ...
        {/* system prompt context — collapsible banner */}
        {hasSystem && (
            ...
            <div className="overflow-hidden rounded-lg border border-dashed border-border bg-muted/30">
              <CollapsibleTrigger className="flex w-full items-center gap-2 px-2.5 py-1.5 text-left hover:bg-muted/50">
                ...
                <span className="truncate text-[10px] text-muted-foreground/70">
                  ...
                  {(message.systemPrompt!.split("\n")[0].length > 60 || message.systemPrompt!.includes("\n")) && "…"}
                </span>
                <ChevronRight
                  className={cn(
